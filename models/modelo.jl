...
files = [p*"c_v10_a45_d4.txt", p*"c_v15_a105_d4.txt", p*"c_v20_a190_d4.txt", p*"c_v25_a300_d4.txt", p*"c_v25_a300_d6.txt"]
...
for i in 1:length(files)
    ...
    g_params.max_cuts = ceil(0.2*ins.n)
    ...
    set_optimizer_attribute(model, "Heuristics", 0) # Desabilitar cortes
    ...
    append!(tempo_total, tempo)
    append!(nodes_total, nodes)
    append!(obj_total, obj)

    # results(obj, 0, tempo, files[i], ins.B, ins.L)
end

println("Tempo total: ",  tempo_total)
println("Número de nós: ", nodes_total)
println("Bounds: ", obj_total)